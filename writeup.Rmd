...
This assignment describes the construction of a machine learning algorithm for Human Actitivy Recognition (HAR). The data is taken from [linked phrase](http://example.com) and was measured on 5 individuals. The test subjects were asked to perform a physical exercise in 5 (incorrect) different manners. We will construct an algorithm to predict in which manner the exercise was performed.

## Data Clean-Up and Choice of Predictors

The data consists of two seperate data sets, one for training (consisting of 19622 observations of 160 variables) and one for the testing data (consisting of 20 observations of 160 variables). First, we import the training data by
...
Furthermore we remove some variables which do not contain any information about the movement data (e.g. recording time, name of the subject etc.) and convert the remaining columns to appropriate types:
...
We can see that our estimated out-of-sample error is around 62 % which is obviously too high to yield any reliable predictions from the model. In the following we will therefore switch to another, more elaborate model, the Random Forest model.
...
We will now go ahead and train the model with the full PCA training data set and make the predictions on the testing data set in file _pml-testing.csv_.
...
The data was validated during an online submission interface and achieved an accuracy of 95 % (1 out of 20 predictions were wrong). Using a Random Forest model trained on the full 52 predictor data set the one erroneous prediction could be corrected.